...
class Lesson (object):
    ...
    def __str__(self):
        return '%s מלמד/ת %s ביום %s בשעה %s בכיתה %s' % (self.teacher,
                                                          self.name,
                                                          self.DAYS[self.day],
                                                          self.period,
                                                          self.classroom)
...
class Bot (object):
    ...
    def __init__(self):
        try:
            with open(CACHE, 'rb') as cachef:
                self.cache = pickle.load(cachef)
        except:
            self.save_cache()
            with open(CACHE, 'rb') as cachef:
                self.cache = pickle.load(cachef)
        # self.cache = {'classes': {num: name}, 'lessons': [Lesson]}

    def get_teacher_schedule(self, teacher, day):
        return sorted(filter(lambda x: x.day == day and teacher in x.teacher, self.cache['lessons']), key=lambda x: x.period)

    def save_cache(self):  # ! This will send web requests
        classes = self.fetch_classes()  # {num: name}
        lessons = list()
        for classnum in classes.keys():
            lessons += self.fetch_lessons(classnum)
        with open(CACHE, 'wb') as cachef:
            pickle.dump({'classes': classes, 'lessons': lessons}, cachef)

    def fetch_classes(self):
        """:returns: {classnum: classname}"""
        soup = BeautifulSoup(self.fetch_page(), 'html.parser')
    ...
    def fetch_lessons(self, classnum):
        soup = BeautifulSoup(self.fetch_page(classnum), 'html.parser')
        ...
        for rownum, row in enumerate(rows[1:]):
            for colnum, col in enumerate(row.findChildren('td', recursive=False)[1:]):
                for lessondiv in col.findAll('div', {'class': 'TTLesson'}):
                    lessons.append(Lesson(lessondiv.find('b').text,
                                          lessondiv.find('br').nextSibling,
                                          lessondiv.find('b').nextSibling,
                                          colnum + 1,
                                          rownum))
                    print(lessons[-1])
        return lessons
        
        print([[col.findAll('div', {'class': 'TTLesson'}) for col in row.findChildren('td', recursive=False)[1:]]
                for row in rows[1:]][1][0][0])

    def fetch_page(self, classnum=0):  # ! This will generate web requests
